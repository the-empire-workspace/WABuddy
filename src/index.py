...
class ExcelApp(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle("Excel Loader - PoC")
    ...
    def process_ia_response(self):
        ...
        for index, row in self.excel_data.iterrows():
            send_time = (current_time.hour, current_time.minute)
            prompt = f'''
            ...
            worker = WhatsAppWorker(row, prompt, self.update_progress(), send_time)
    ...
    def update_progress(self):
        def handle_result(row, response, whatsapp_active):
            def update_ui():
                ...
                self.table.setItem(index, 5, QTableWidgetItem(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def show_excel_data_dynamic(self):
        ...
        self.export_button = build_button("Exportar a Excel", on_click=export_to_excel(self))
